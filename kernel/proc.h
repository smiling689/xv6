...
struct proc {
  ...
  int pid;                     // Process ID
  int trace_mask;              // Trace mask for syscalls
...
#ifdef LAB_PGTBL
  struct usyscall *usyscallpage; // 用户态只读共享页
#endif
  ...
  int alarm_handler_running;   // 避免 handler 重入
  struct trapframe alarm_tf;   // 保存被 alarm 打断时的用户态以供恢复
};
